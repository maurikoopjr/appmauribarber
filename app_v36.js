...
function renderizarDevDashboard() {
    try {
        ...
        let totalTrial = tenants.filter(t => t && t.status === "trial").length;
        let totalExpirados = tenants.filter(t => t && t.status === "expired").length;
        let faturamentoSaaS = 0;

        tenants.forEach(t => {
            if (t && t.status === "active") {
                const plan = plans.find(p => p && p.id === t.planId);
                if (plan) {
                    const price = parseFloat(plan.price || 0);
                    if (!isNaN(price)) faturamentoSaaS += price;
                }
            }
        });
        ...
        console.error("Erro ao renderizar Dev Dashboard:", err);
    }
}
...
function renderizarDevTenants() {
    try {
        ...
        tbody.innerHTML = tenants.map(t => {
            if (!t) return "";
            const plan = plans.find(p => p && p.id === t.planId) || { name: "Sem Plano" };
            const expTime = t.status === "trial" ? t.trialExpires : t.planExpires;
            
            let expDate = "—";
            try {
                if (expTime) {
                    const dateObj = new Date(expTime);
            ...
            return `
                <tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
                    <td style="padding:12px; font-weight:600; color:var(--text-primary);">${t.name || "Sem Nome"}</td>
                    ...
                    <td style="padding:12px; font-size:13px; color:var(--accent-gold); font-weight:500;">${plan.name || "Sem Plano"}</td>
        ...
        console.error("Erro ao renderizar Dev Tenants:", err);
        const tbody = document.getElementById("devTenantsTableBody");
        if (tbody) {
            tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; padding:20px; color:var(--accent-danger);">Erro ao carregar barbearias: ${err.message}</td></tr>`;
        }
    }
}
...
function renderizarDevPlans() {
    try {
        ...
        console.error("Erro ao renderizar Dev Plans:", err);
        const tbody = document.getElementById("devPlansTableBody");
        if (tbody) {
            tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:20px; color:var(--accent-danger);">Erro ao carregar planos: ${err.message}</td></tr>`;
        }
    }
}
